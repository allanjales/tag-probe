...
void invariantMassProbe()
{
	...
	TTree *Tree = (TTree*)file0->Get("demo/PlotControl");	//Opens TTree of file
	...
	TH1D *hPtSigBack  = new TH1D("ProbeMuon_PtSigBack",		"Transversal Momentum (Probe);Transversal Momentum (GeV/c);Events",100,0.,100.);
	...
	//Create canvas
	TCanvas *c1 = new TCanvas("ProbeSignal_InvariantMass","Invariant Mass", 600, 600);			//Creates Canvas for drawing
	...
	//Create canvas
	TCanvas *c2 = new TCanvas("ProbeSignal_Pt","Transversal Momentum", 1200, 600);
	...
	hPtSigBack->SetLineWidth(2);		//Line Width
	hPtSigBack->Draw();

	//Draws Background histogram
	hPtBack->SetLineColor(kBlue); 		//Line Color
	hPtBack->SetLineStyle(kDashed);		//Line Style
	hPtBack->SetLineWidth(2);			//Line Width
	hPtBack->Draw("same");

	//Draws Signal histogram
	hPtSig->SetLineColor(kMagenta); 	//Line Color
	hPtSig->SetLineWidth(2);			//Line Width
	hPtSig->Draw("same");
	...
	//Create canvas
	TCanvas *c3 = new TCanvas("ProbeSignal_Eta","Pseudorapidity", 1200, 600);
	...
	//Draws Main histogram
	hEtaSigBack->SetLineWidth(2);		//Line Width
	hEtaSigBack->Draw();

	//Draws Signal histogram
	hEtaSig->SetLineColor(kMagenta); 	//Line Color
	hEtaSig->SetLineWidth(2);			//Line Width
	hEtaSig->Draw("same");

	//Draws Background histogram
	hEtaBack->SetLineColor(kBlue); 		//Line Color
	hEtaBack->SetLineStyle(kDashed);	//Line Style
	hEtaBack->SetLineWidth(2);			//Line Width
	hEtaBack->Draw("same");
	...
	*/
	...
	//Create canvas
	TCanvas *c4 = new TCanvas("ProbeSignal_Phi","Phi", 1200, 600);
	...
	hPhiSigBack->SetLineWidth(2);		//Line Width
	hPhiSigBack->Draw();

	//Draws Signal histogram
	hPhiSig->SetLineColor(kMagenta); 	//Line Color
	hPhiSig->SetLineWidth(2);			//Line Width
	hPhiSig->Draw("same");

	//Draws Background histogram
	hPhiBack->SetLineColor(kBlue); 		//Line Color
	hPhiBack->SetLineStyle(kDashed);	//Line Style
	hPhiBack->SetLineWidth(2);			//Line Width
	hPhiBack->Draw("same");
